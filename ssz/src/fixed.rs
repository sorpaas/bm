...
macro_rules! impl_builtin_fixed_uint_vector {
    ( $( $t:ty ),* ) => { $(
        impl<'a, DB> SerializeVector<DB> for Serial<'a, FixedVec<$t>> where
...
impl<'a, DB> SerializeVector<DB> for Serial<'a, FixedVec<U256>> where
    DB: Backend<Intermediate=Intermediate, End=End>
{
    fn serialize_vector(&self, db: &mut DB, at_depth: Option<usize>) -> Result<ValueOf<DB>, Error<DB::Error>> {
...
{
    fn serialize_vector(&self, db: &mut DB, at_depth: Option<usize>) -> Result<ValueOf<DB>, Error<DB::Error>> {
        Serial(&FixedVec(self.0.as_ref().iter().cloned().collect())).serialize_vector(db, at_depth)
    }
}

impl<'a, T: Composite, DB> SerializeVector<DB> for Serial<'a, FixedVec<T>> where
